...
abstract contract CookieJar6551 is Initializable, OwnableUpgradeable {
    ...
    function setUp(bytes memory _initializationParams) public virtual {
        (address payable _target, uint256 _periodLength, uint256 _cookieAmount, address _cookieToken) =
    ...
    function giveCookie(address cookieMonster, uint256 amount) private {
        ...
        if (cookieToken == address(0)) {
            target.executeTrustedCall(SUSTAINABILITY_ADDR, fee, bytes(""));
            target.executeTrustedCall(cookieMonster, amount - fee, bytes(""));
        } else {
            target.executeTrustedCall(
                cookieToken,
                0,
