...
contract CookieJar6551Factory {
    ...
    */
    ...
    function summonCookieJar(
        ...
        returns (address)
    {
        CookieJar6551 _cookieJar = CookieJar6551(Clones.clone(_singleton));
        _cookieJar.setUp(_initializer);

        CookieJar6551(_cookieJar).transferOwnership(msg.sender);

        emit SummonCookieJar(address(_cookieJar), details, _initializer);
        return address(_cookieJar);
    }
}
