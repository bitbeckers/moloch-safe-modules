...
import { ERC721CookieJar } from "./ERC721CookieJar.sol";
...
contract CookieJarFactory {
    ...
    function summonBaalCookieJar(
        ...
        public
    {
        bytes memory _initializer = abi.encode(_safeTarget, _dao, _threshold, _useShares, _useLoot);

        BaalCookieJar _cookieJar = BaalCookieJar(Clones.clone(baalCookieJarSingleton));
        _cookieJar.setUp(_initializer);

        emit SummonCookieJar(_dao, address(_cookieJar), bytes("baal"), _cookieAmount, _periodLength, _cookieToken);
    }
    ...
    function summonErc20CookieJar(
        ...
        public
    {
        bytes memory _initializer = abi.encode(_safeTarget, _erc20Addr, _threshold);

        ERC20CookieJar _cookieJar = ERC20CookieJar(Clones.clone(erc20CookieJarSingleton));
        _cookieJar.setUp(_initializer);

        emit SummonCookieJar(
            _safeTarget, address(_cookieJar), bytes("erc20"), _cookieAmount, _periodLength, _cookieToken
        );
    }
    ...
    function summonErc721CookieJar(
        ...
        public
    {
        bytes memory _initializer = abi.encode(_safeTarget, _erc721Addr);

        ERC721CookieJar _cookieJar = ERC721CookieJar(Clones.clone(erc721CookieJarSingleton));
        _cookieJar.setUp(_initializer);

        emit SummonCookieJar(
            _safeTarget, address(_cookieJar), bytes("erc721"), _cookieAmount, _periodLength, _cookieToken
        );
    }
        ...
        public
    {
        ...
        emit SummonCookieJar(
            _targetSafe, address(_cookieJar), bytes("none"), _cookieAmount, _periodLength, _cookieToken
        );
    }
